        ...
        <div class="container">
            ...
            <div class="page__text flex_row">
                ...
                <div class="entity__data flex-column">
                    ...
                    <form name='groups_form' action="{{ route('admin.faux.update', $student->id) }}" method='post'
                          class="student__form">
                        @csrf
                        @method('patch')
                            ...
                            <div class="add_venue">
                                ...
                                <select name='group_id'>
                                    ...
                                    @foreach($groups as $group)
                                        <option
                                            value="{{ $group->id }}" @selected(old('group_id') == $group->id)>{{ $group->name }}</option>
                                    @endforeach
                                </select>
                        ...
                        <input type='text' name='last_name' placeholder="Last Name"
                               value="{{ old('last,name', $student->last_name) }}"/>
                        ...
                        <select name='course_ids[]' multiple>
                            @foreach($courses as $course)
                                <option
                                    value="{{ $course->id }}" @selected(in_array($course->name, $studentCourses, true))>{{ $course->name }}</option>
                            @endforeach
                        </select>
                        <p class="select_note">Hold down the Ctrl (windows) or Command (Mac) button to select multiple
                            options.</p>
